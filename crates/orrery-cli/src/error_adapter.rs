//! Error adapter for converting OrreryError to miette diagnostics.
//!
...
//! # Multi-Error Support
//!
//! When a [`orrery_parser::error::ParseError`] contains multiple diagnostics, each
...
/// Adapter for non-diagnostic [`OrreryError`] variants.
///
...
/// Convert an orrery [`Span`](orrery_parser::Span) to a miette [`SourceSpan`].
fn span_to_miette(span: orrery_parser::Span) -> SourceSpan {
    SourceSpan::new(span.start().into(), span.len())
}

/// Convert an [`OrreryError`] into a list of reportable errors.
///
/// For [`OrreryError::Parse`], this returns one [`Reportable`] for
