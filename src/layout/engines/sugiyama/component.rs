...
#[cfg(test)]
mod tests {
    use crate::{ast, color::Color, graph::Graph, layout::engines::ComponentEngine};
    ...
    #[test]
    fn test_sugiyama_layout_with_cycles() {
        // Create a graph with a cycle
        let mut diagram = ast::Diagram {
        ...
        let default_type = ast::TypeDefinition::defaults()[0].clone();

        // Create test nodes
        let node1 = ast::Node {
            id: ast::TypeId::from_name("a"),
            name: String::from("a"),
            display_name: Some("A".into()),
            type_definition: default_type.clone(),
            block: ast::Block::None,
        };
        let node2 = ast::Node {
            id: ast::TypeId::from_name("b"),
            name: String::from("b"),
        ...
        diagram.scope.elements.push(ast::Element::Node(node1));
        diagram.scope.elements.push(ast::Element::Node(node2));
        diagram.scope.elements.push(ast::Element::Node(node3));

        // Create relations with a cycle: a -> b -> c -> a
        let rel1 = ast::Relation {
            source: ast::TypeId::from_name("a"),
            target: ast::TypeId::from_name("b"),
            relation_type: ast::RelationType::Forward,
            color: Color::default(),
            width: 1,
            arrow_style: ast::ArrowStyle::default(),
            label: None,
        };
        let rel2 = ast::Relation {
            source: ast::TypeId::from_name("b"),
            target: ast::TypeId::from_name("c"),
            relation_type: ast::RelationType::Forward,
            color: Color::default(),
            width: 1,
            arrow_style: ast::ArrowStyle::default(),
        ...
        let rel3 = ast::Relation {
            source: ast::TypeId::from_name("c"),
            target: ast::TypeId::from_name("a"),
            relation_type: ast::RelationType::Forward,
            color: Color::default(),
            width: 1,
            arrow_style: ast::ArrowStyle::default(),
        ...
        diagram.scope.elements.push(ast::Element::Relation(rel1));
        diagram.scope.elements.push(ast::Element::Relation(rel2));
        diagram.scope.elements.push(ast::Element::Relation(rel3));

        let graph = Graph::from_diagram(&diagram).expect("Failed to create cyclic test graph");
        let engine = Engine::new();

        // This shouldn't panic, even with cycles
        let layout = engine.calculate(&graph);

        // Check that all nodes and relations are present
    ...
    fn test_engine_basic() {
        ...
        diagram.scope.elements.push(ast::Element::Relation(rel));

        // Create graph from diagram
        let graph = Graph::from_diagram(&diagram).expect("Failed to create test graph");

        // Calculate layout
        let layout = engine.calculate(&graph);

        // Verify that the layout has the correct number of components and relations
        assert_eq!(
            layout.components.len(),
            2,
            "Expected 2 components in the layout"
        );
        assert_eq!(
            layout.relations.len(),
            1,
            "Expected 1 relation in the layout"
        );

        // Verify that components are properly positioned
        // Find components by comparing display names
        let find_component_by_display_name = |name: &str| {
            layout
                .components
                .iter()
                .find(|c| c.node.display_name.as_deref() == Some(name))
        };

        let comp_a = find_component_by_display_name("A").expect("Component A not found");
        let comp_b = find_component_by_display_name("B").expect("Component B not found");

        // Either we're using Sugiyama layout (which might position nodes in any order)
        // or we're using the fallback layout (which should position nodes properly)

        // We only verify that positions are assigned, not their specific values
        assert!(
            comp_a.position.x != 0.0 || comp_a.position.y != 0.0,
            "Component A position not initialized"
        );
        assert!(
            comp_b.position.x != 0.0 || comp_b.position.y != 0.0,
            "Component B position not initialized"
        );

        // Verify component sizes
        assert!(comp_a.size.width >= 100.0, "Component A width is too small");
        assert!(
            comp_a.size.height >= 60.0,
            "Component A height is too small"
        );
    }
}
